...
function setupEventListeners() {
    ...
    // Test Jupiter API button
    const testJupiterBtn = document.getElementById('test-jupiter-btn');
    if (testJupiterBtn) {
...
async function loadJupiterTokens() {
    ...
    try {
        console.log('Loading Jupiter tokens...');
        updateStatus('Loading token directory...');
        
        // Fetch verified tokens (smaller payload than all tokens)
        ...
        console.log(`Loaded ${jupiterTokens.length} verified tokens from Jupiter`);
        updateStatus('Token directory loaded');
        
        return jupiterTokens;
    } catch (error) {
        console.error('Error loading Jupiter tokens:', error);
        updateStatus('Failed to load tokens');
        throw error;
    }
}
...
async function searchTokenBySymbol(symbol) {
    try {
        const tokens = await loadJupiterTokens();
...
async function getTokenPrice(mintAddress) {
    try {
        ...
        if (data.data && data.data[mintAddress]) {
            ...
            updateStatus('Price fetched successfully');
            return priceInfo;
        } else {
            console.log('No price data found for token');
            updateStatus('No price data available');
            return null;
        }
    } catch (error) {
        console.error('Error fetching token price:', error);
        updateStatus('Price fetch failed');
        return null;
    }
}
...
async function testJupiterAPI() {
    try {
        console.log('🧪 Testing Jupiter API workflow...');
        
        // Test with JUP token
        const token = await searchTokenBySymbol('JUP');
        if (token) {
            console.log('Found JUP token:', token);
            
            const price = await getTokenPrice(token.address);
            if (price) {
                console.log('JUP price:', price.price, 'USDC');
            }
        }
        
        // Test with SOL
        const solToken = await searchTokenBySymbol('SOL');
        if (solToken) {
            console.log('Found SOL token:', solToken);
            
            const solPrice = await getTokenPrice(solToken.address);
            if (solPrice) {
                console.log('SOL price:', solPrice.price, 'USDC');
            }
        }
...
if (typeof window !== 'undefined') {
    window.jupiterPopup = {
        ...
        testJupiterAPI // Expose test function
    };
}
